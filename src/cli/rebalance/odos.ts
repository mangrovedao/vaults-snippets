...
import type { OdosRebalanceEntry, RegistryEntry } from "../../registry";
...
export async function rebalanceOdos(
  client: Client,
  vault: Address,
  vaultState: CurrentVaultState,
  rebalance: OdosRebalanceEntry,
  ...
  const spinner = ora(`Getting quote on odos...`).start();
  ...
  spinner.succeed(
    `Quote: ${formattedAmountIn} ${buyToken.symbol} for ${formattedAmountOut} ${
      sellToken.symbol
    }; price: ${(action === "buy"
  ...
  const built = await buildOdosSwap(vault, quote.pathId);
